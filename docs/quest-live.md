...
2. 세션 이름과 작업 지시를 입력하고 세션을 생성한다. 작업·Quest 식별자는 자동으로 생성한다.
3. 생성 직후 수집 콘솔에 입장한다. **장치 → Quest 연결**을 열어 6자리 연결 코드를 확인한다.
4. Quest 브라우저에서 같은 서버에 연결되는 HTTPS 사이트의 `/collect/quest`를 열어 코드를 입력하고 **연결**을 누른다.
5. Quest에서 **손 추적 시작**을 누르고 권한 요청을 허용한다. 기기의 손 추적을 켜고 양손이 인식되도록 둔다.
6. 연결되면 PC의 연결 창이 자동으로 닫히고 장치 사이드바에 Quest가 표시된다. 움직이는 양손 관절을 확인하고 **Episode 녹화 시작**을 누른다.
7. 녹화를 정지한 뒤 Quest의 남은 프레임 전송이 끝나면 녹화본을 저장한다. 다음 Episode를 같은 수집에서 이어서 기록할 수 있다.

연결 버튼을 누를 때 새 코드를 발급하며 서버 발급 시점부터 5분 동안 한 번만 사용할 수 있다. 화면에서 자동 재발급하지 않는다. 코드가 만료되거나 Quest 페이지를 새로고침한 경우 해당 PC 수집 세션에서 코드를 갱신하고 다시 연결한다. gateway 재시작 후에도 수집 기록은 남지만 Quest는 다시 페어링해야 한다.
...
PC의 코드 발급과 손 추적 확인은 수집 콘솔에서 수행한다. `/collect/quest`는 Quest 기기의 코드 입력·MR 실행 전용이다. 기존 `/collect/quest?view=pc` 주소는 `/mlops/collection`으로 이동한다.
...
수집 브라우저 검증은 자체 임시 gateway와 Real frontend를 실행하고 종료한다. 별도 PC·Quest 브라우저 컨텍스트에서 **새 수집 → 세션 생성 → 수집 콘솔 → Quest 연결·페어링 → 녹화 → 정지·저장 → PC 새로고침**을 수행하고 실제 NDJSON 파일 생성을 확인한다. 화면 캡처와 원본은 `artifacts/quest-collection-test-<timestamp>/`에 생성한다.
