...
- 공통 조회 오류·빈 상태는 `Surface density="compact"`의 정적 업무 표면에 안내 한 번과 필요한 재시도만 표시한다. 오류는 공통 상태 아이콘·본문·우측 보조 버튼으로 정렬하며, 중복 제목이나 내부 오류색 박스를 추가하지 않는다. 긴 안내는 줄바꿈하고 버튼의 최소 조작 영역은 유지한다.
