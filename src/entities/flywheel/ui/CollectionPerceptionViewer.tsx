...
export function CollectionPerceptionViewer({ result }: { readonly result: CollectionHeadPerception | null }) {
  const [failedUrl, setFailedUrl] = useState<string | null>(null);
  const failed = result !== null && (failedUrl === result.depthImageUrl || failedUrl === result.segmentationMaskUrl);
  return (
    <FittedMedia constrained={result !== null && !failed}>
      <MediaPanel
        className="h-auto grid-rows-[auto_auto]"
        aria-label="Head RGB Depth와 세그멘테이션"
        data-perception-viewer
        title="Head Depth + 분할"
        status={result === null || failed ? <StatusIndicator className="text-xs" label={failed ? '불러오기 실패' : '결과 대기'} tone={failed ? 'negative' : 'neutral'} /> : null}
      >
        <div className={`relative flex items-center justify-center overflow-hidden ${result !== null && !failed ? 'aspect-video' : ''}`} data-aspect-media-viewport data-perception-viewport>
          {result === null ? <p className="p-4 text-center text-xs leading-relaxed text-muted">Head RGB의 깊이·세그멘테이션 결과를 기다립니다.</p> : failed ? (
