...
function RobotMonitoringContent({ robot }: { readonly robot: RobotDescriptor }) {
  return (
    <div className="flex h-dvh min-h-0 flex-col overflow-hidden bg-neutral-100">
      ...
      <div className="grid min-h-0 flex-1 place-items-center bg-neutral-900 p-4 text-white">
        <p role="status">영상 관제 기능을 준비하고 있습니다.</p>
      </div>
    </div>
  );
}
...
export function RobotMonitoringPage() {
  ...
  if (robot.status === 'loading') {
    return <div className="min-h-dvh bg-neutral-100 p-4 sm:p-6"><QueryFeedback kind="loading" /></div>;
  }
  if (robot.status === 'error') {
    return <div className="min-h-dvh bg-neutral-100 p-4 sm:p-6"><QueryFeedback kind="error" message={robot.message} onRetry={robot.retry} /></div>;
  }
  if (robot.data === null) {
    return (
      <div className="grid min-h-dvh content-start gap-4 bg-neutral-100 p-4 sm:p-6">
        ...
        <Link aria-label="모니터링으로 돌아가기" className="inline-flex size-10 items-center justify-center rounded-md" to="/control/monitoring">
          <Icon name="back" />
        </Link>
      </div>
    );
